...
import type { PluginContext } from "../../plugin/utils/types.js";
import { assertUsage } from "../../utils/assert.js";

export function updateVercelEntry<T extends Photon.Entry>(entry: T): T {
  if (!entry.vikeMeta) return entry;
...
export function setTargetVercel(pluginContext: PluginContext) {
  for (const entry of pluginContext.environment.config.photon.entries) {
    updateVercelEntry(entry);
  }

  // Configure server entry
  {
    const vikeConfig = getVikeConfig(pluginContext.environment.config);
    // More readable name than the default one
    const name = `${pageNamePrefix}/__catch_all`;
    pluginContext.environment.config.photon.server.route = "/**";
    pluginContext.environment.config.photon.server.vercel = {
